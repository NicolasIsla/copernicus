...
class GlobalLocalAttention(nn.Module):
    ...
    def pad(self, x, ps):
        _, _, H, W = x.size()
        right_pad = ps - W % ps if W % ps != 0 else 0
        bottom_pad = ps - H % ps if H % ps != 0 else 0
        if right_pad > 0 or bottom_pad > 0:
            x = F.pad(x, (0, right_pad, 0, bottom_pad), mode='reflect')
        return x
...
class SFANet(nn.Module):
    def __init__(
        self,
        in_channels=11,              # tus bandas numéricas
        cat_num_categories=15,       # 11 categorías en el canal categórico
        cat_emb_dim=4,               # tamaño del embedding, puedes ajustarlo (2, 4, 8, 16, etc)
        decode_channels=64,
        dropout=0.1,
        backbone_name='efficientnet_b5', #'efficientnet_b3'
        pretrained=True,
        window_size=8,
        num_classes=24
    ):
        super().__init__()
        self.categorical_embedding = nn.Embedding(cat_num_categories, cat_emb_dim)
        self.backbone = timm.create_model(
            ...
            in_chans=in_channels + cat_emb_dim
        )
    ...
    def forward(self, x, cat):
        """
        x: (B, 11, H, W)  -- bandas numéricas
        cat: (B, H, W)    -- categórico sparse (long)
        """
        cat_emb = self.categorical_embedding(cat)         # (B, H, W, emb_dim)
        cat_emb = cat_emb.permute(0, 3, 1, 2)             # (B, emb_dim, H, W)
        x_aug = torch.cat([x, cat_emb], dim=1)            # (B, 11+emb_dim, H, W)
        h, w = x_aug.size()[-2:]
        enc1, enc2, enc3, enc4 = self.backbone(x_aug)
        x_out, ah = self.decoder(enc1, enc2, enc3, enc4, h, w)
        return x_out, ah