...
class SFANetPretrained(SFANet):
    def __init__(
        ...
        super().__init__(
            ...
            pretrained=False  # important: do not load ImageNet here
        )
...
class LazySatelliteDataset(Dataset):
    def __init__(self, npz_paths, label_paths):
        ...
        self.num_classes = 24  # adjust if needed
...
class BalancedTrainingManager:
    ...
    def train_epoch(self, train_loader, epoch_idx):
        ...
        for batch_idx, (x, cat, y) in enumerate(train_loader):
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                ...
                conf_penalty = self.confusion_penalty if self.use_confusion_loss else None
                loss = self._hybrid_loss(outputs[0], y, confusion_matrix=conf_penalty)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for batch_idx, (x, cat, y) in enumerate(val_loader):
                ...
                with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                    outputs = self.model(x, cat)
                    # en valid no usamos penalización por confusión
                    loss = self._hybrid_loss(outputs[0], y, confusion_matrix=None)
    ...
    def _finalize_training(self, history):
        # No hardware plots here, only model metrics
...
if __name__ == "__main__":
    ...
    # DataLoader
    BATCH_SIZE        = 6      # 6 para EfficientNet-B5
